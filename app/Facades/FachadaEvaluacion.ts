...
export default class FachadaEvaluacion{
  ...
  public async listarEvaluacionesId({request, response}: HttpContextContract){
    const api = new Api()
    const evaControlador = new EvaControlador()
    const num_doc = request.input("eva_id")

    const result = await evaControlador.indexById(num_doc)
  ...
  public async actualizarResultado({request, response}: HttpContextContract){
    const api = new Api()
    const data = request.only(['eva_id','eva_resultado','eva_puntaje', 'eva_estado'])
    const evaControlador = new EvaControlador()
